...
public class WidokKsiazka extends Panel {
    public ZobaczStatusEnum status = ZobaczStatusEnum.books;
    protected String strona;
    protected WebView widok;
    ...
    public void onActivityCreated(Bundle saved) {
        ...
        widok = (WebView) getView().findViewById(R.id.Viewport);
        
        widok.getSettings().setJavaScriptEnabled(true);
//Przewracanie
        widok.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {

                if (status == ZobaczStatusEnum.books)
                    swipePage(v, event, 0);
    ...
    @Override
    public void saveState(SharedPreferences.Editor edytor) {
        super.saveState(edytor);
        edytor.putString("state"+index, status.name());
        edytor.putString("page"+index, strona);
    }
    ...
    public void loadState(SharedPreferences preferences)
    {
        ...
        status = ZobaczStatusEnum.valueOf(preferences.getString("state"+index, ZobaczStatusEnum.books.name()));
    }
